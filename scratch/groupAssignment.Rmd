...
```{r message = F}
library(dplyr)
df <- read.csv("../data/golfer_solution_test.csv")
...
candidates <- generatePermutation(2, 2)
...
Conflict <- function(round1_matrix, mutation_weights) {
  ...
  combined_matrices <- lapply(mutation_weights, function(candidate_matrix) {
    ...
    return(list(round1_matrix, candidate_matrix))  # List of round 1 and cumulative matrix for each candidate
  })
...
matrix3 <- matrix(c(
  -99,  0,   2,   1,
    ...
    2,   1, -99,   0,
    1,   2,   0, -99
), nrow = 4, ncol = 4, byrow = TRUE, dimnames = dimnames_list)

# Define the fourth edited matrix with -99 on the diagonal
matrix4 <- matrix(c(
  -99,  1,   1,   1,
    1, -99,   1,   1,
    1,   1, -99,   1,
