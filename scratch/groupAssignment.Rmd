...
updatemat <- function(file_path, rounds) {
  ...
  # Extract the specified round of group assignments
  round_col <- data[[round + 1]] # Adjust for the correct column index
  
  # Update the weight matrix based on the group assignments
...
conflict_per_round <- function(file_path, rounds) {
  ...
  for (i in seq_along(rounds)) {
    round <- rounds[i]
    weight_matrix <- weight_matrix + updatemat(file_p ath, round)
